...
function generateViralTitle() {
  const type = Math.floor(Math.random() * 3);
  if (type === 0) {
    return `दुनिया के ${rand(numbers)} सबसे ${rand(adjectives)} ${rand(nouns)}, ${rand(reactions)}`;
  } else if (type === 1) {
    return `क्या सच में ${rand(myths)}? ${rand(experts)} ने किया चौंकाने वाला खुलासा!`;
  } else {
...
async function seed() {
  console.log("Generating 1050 viral articles...");
  
  const uniqueTitles = new Set<string>();
  while(uniqueTitles.size < 1050) {
    uniqueTitles.add(generateViralTitle());
  }
  ...
  for (let i = 0; i < total; i += batchSize) {
    ...
    const insertData = batch.map((title, index) => {
      const category = categories[Math.floor(Math.random() * categories.length)];
      const imageUrl = categoryImages[category as keyof typeof categoryImages];
      const baseSlug = slugify(title, { lower: true, strict: true, locale: 'hi' }) || `viral-article-${Date.now()}`;
      const slug = baseSlug + '-' + Math.random().toString(36).substring(2, 8);
      
      const date = new Date();
      date.setDate(date.getDate() - Math.floor(Math.random() * 365)); // past 1 year
      date.setHours(Math.floor(Math.random() * 24));

      return {
        title: title,
        slug: slug,
        content: contentTemplates[category as keyof typeof contentTemplates],
        category: category,
        imageUrl: imageUrl,
        seoTitle: title + " - अनसुने रहस्य",
        seoDescription: title + " के बारे में पूरी जानकारी और अनसुने रहस्य जो आपको जरूर जानने चाहिए।",
        keywords: category + ", viral, trending, mystery, facts",
        status: "published",
        publishedDate: date,
    ...
    try {
      ...
      console.log(`Inserted ${count}/${total} articles...`);
    } catch (e) {
      console.error("Error inserting batch:", e);
    }
  }

  console.log(`Successfully seeded ${count} highly viral articles!`);
  process.exit(0);
}
